...
public class RegistroController {
	...
	public ModelAndView consultarRegistrosFechados() {
		...
		if(registrosFinalizados.isEmpty()) {
			...
			modelAndView.addObject("mensagemErro", "N�o existe nenhum hist�rico de registros.");
		}else {
	...
	public ModelAndView inserirRegistro(@Valid BaseRegistro registro, BindingResult bindingResult, RedirectAttributes redirectAttributes) {
		...
		if(registroDAO.existeRegistroAtivo(registroFinal)) {
			redirectAttributes.addFlashAttribute("mensagemErro", "O Aluno " + registroFinal.getAluno().getRegistroAluno() + " j� tem registro de entrada");
		}else {
			...
			redirectAttributes.addFlashAttribute("mensagemSucesso", "Registro de entrada conclu�do para o aluno " + registroFinal.getAluno().getRegistroAluno());
		}
	...
	public ModelAndView atualizarRegistroComSaida(@Valid BaseRegistro registro, BindingResult bindingResult, RedirectAttributes redirectAttributes) {
		...
		redirectAttributes.addFlashAttribute("mensagemSucesso", "Registro de sa�da conclu�do para o aluno " + registro.getRegistroAluno());
		return modelAndView;
	}
